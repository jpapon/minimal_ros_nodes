...
class FCN8VGG:
    ...
    def _conv_layer(self, bottom, name):
        with tf.variable_scope(name) as scope:
            shape = np.array(self.data_dict[name][0].shape)
            #If input layer, don't use VGG weights
            if False: #(name == "conv1_1"):
              shape[2] = 1
              print ("-"*50)
              print('Layer conv1_1 shape: %s' % str(shape))
              print ("-"*50)
              #filt = self.get_conv_filter_random_init(name,tuple(shape))
              conv_biases = self.get_bias_random_init(name)
              filt = self.get_conv_filter_1(name)
